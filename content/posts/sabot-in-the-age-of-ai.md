...
<div class="caption"><img src="output-405.png">A captured screenshot of a page generated by a tarpit deployed as part of the ongoing <a href="https://algorithmic-sabotage.github.io/asrg/trapping-ai/#expanding-the-offensiveness">‘Trapping AI’</a> operation. The tarpit, built on <a href="https://git.jsbarretto.com/zesterer/babble">Babble</a> — a standalone LLM crawler-tarpit binary — generates an endless stream of deterministic bollocks that traps crawlers on a single site and forces them to endlessly navigate a constantly expanding sea of pages 🌊. Each new page spawns additional layers of meaningless text and dozens of links, dragging them ever deeper into the tarpit 🕳️.</div>
...
| 17 | [HTML Zip Bomb](https://ache.one/bomb.html) | N/A | [URL](https://ache.one/notes/html_zip_bomb) | A valid HTML page containing a *zip bomb* using `gzip` and `brotli` compression. Its goal is to exhaust the memory of web crawlers by overwhelming them with massive decompression loads. |
| 18 | Sarracenia | [AGPLv3](https://github.com/CTAG07/Sarracenia/blob/main/LICENSE) | [URL](https://github.com/CTAG07/Sarracenia) | The goal of `sarracenia` is to serve as a defensive wall against web scrapers by generating endless, *plausible-looking* web pages that trap them in a loop of *fake content*, preventing access to protected pages. |
